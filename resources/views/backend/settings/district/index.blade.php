            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <a href="{{route('district.create')}}" class="btn btn-primary btn-sm"><i class="fas fa-add"></i>{{__('lang.add')}}</a>
                </div>
                <div class="card-body">
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tr>
                      <td>{{$stt++}}</td>
                      <td>{{$d->name}}</td>
                      <td>{{$d->proname->name}}</td>
                      <td>
                        <form action=" {{ route('district.destroy', $d->id) }} " method="POST">
                          @csrf
                          @method('DELETE')

                          <a href="{{ route('district.edit', $d->id) }}" class="btn btn-warning btn-sm"><i class="fas fa-pencil-alt"></i></a>
