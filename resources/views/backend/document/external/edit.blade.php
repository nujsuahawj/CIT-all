      ...
      <div class="row">
        <div class="col-md-4">
          <div class="card">
            ...
            <form method="POST" action="{{route('external_parts.update', $external_parts->id)}}">
                @csrf
                @method('PATCH')
                <div class="card-body">
                    <div class="form-group">
                      <label for="name">{{__('lang.external_partsname')}}</label>
                      <input type="text" class="form-control" name="name" value="{{$external_parts->name}}" placeholder="{{__('lang.external_partsname')}}">
                    </div>
                </div>
